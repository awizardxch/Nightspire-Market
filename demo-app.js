...
/* ---------- HTTP (GET only) ---------- */
/* DEMO MODE: serves embedded sample data, never touches the network. */
async function api(path) {
  const D = window.NIGHTSPIRE_DEMO;
  const [p, qs] = path.split('?');
  const q = new URLSearchParams(qs || '');
  const notFound = (what) => {
    const err = new Error(what + ' not found in demo data');
    err.code = 'not_found'; err.status = 404; throw err;
  };
  if (p === '/v1/health') return JSON.parse(JSON.stringify(D.health));
  if (p === '/v1/offers') {
    let list = D.offers.slice();
    const f = (k) => q.get(k);
    if (f('giveChain')) list = list.filter((o) => o.giveChain === f('giveChain'));
    if (f('wantChain')) list = list.filter((o) => o.wantChain === f('wantChain'));
    if (f('fillMode')) list = list.filter((o) => o.fillMode === f('fillMode'));
    if (f('mediated') === 'true') list = list.filter((o) => o.badges && o.badges.isMediated);
    if (f('mediated') === 'false') list = list.filter((o) => !(o.badges && o.badges.isMediated));
    if (f('fiatOnly') === 'true') list = list.filter((o) => o.badges && o.badges.hasFiatLeg);
    const page = Math.max(1, parseInt(q.get('page') || '1', 10));
    const limit = Math.max(1, parseInt(q.get('limit') || '12', 10));
    return { offers: list.slice((page - 1) * limit, page * limit),
             page, limit, total: list.length };
  }
  let mo = p.match(/^\/v1\/offers\/(.+)$/);
  if (mo) {
    const o = D.offers.find((x) => x.offerId === decodeURIComponent(mo[1]));
    if (!o) notFound('Offer');
    return { offer: JSON.parse(JSON.stringify(o)),
             fills: JSON.parse(JSON.stringify(D.fills[o.offerId] || [])) };
  }
  mo = p.match(/^\/v1\/auctions\/(.+)$/);
  if (mo) {
    const a = D.auctions[decodeURIComponent(mo[1])];
...
/* ---------- boot ---------- */
document.getElementById('relayUrl').value = 'demo bundle — sample data, no relay';
document.getElementById('relayGo').onclick = () => { checkRelay().then(render); }; /* demo: input is informational */
checkRelay();
render();
